...
# Merge price data with BVC metrics
...
# Create a gradient-colored price line using LineCollection
...
# Use a blue-white-red colormap; adjust normalization as needed
norm_bvc = plt.Normalize(-np.max(np.abs(df_merged['bvc'])), np.max(np.abs(df_merged['bvc'])))
...
# Plot EMA and VWAP lines on top
