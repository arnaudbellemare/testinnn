...
class ACDBVC:
    ...
    def eval(self, df_tr: pd.DataFrame, scale=1e4) -> pd.DataFrame:
        ...
        bvc_list = []
        current = 0.0
        for wv in df_tr['weighted_vol']:
            current = current * alpha_exp + wv
            bvc_list.append(current)
...
if prices_bsi['vwap'].iloc[0] == 0 or not np.isfinite(prices_bsi['vwap'].iloc[0]):
    st.warning("VWAP initial value is zero or invalid. Using ScaledPrice for fallback.")
...
ax_bvc.set_title(f"BVC Over Time ({bvc_model})", fontsize=10)
