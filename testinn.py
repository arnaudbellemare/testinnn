...
def fetch_trades_kraken(symbol="BTC/USD", lookback_minutes=1440, limit=43200):
    ...
    # Keep time in seconds as float; we'll convert later.
...
class ACDBVC:
    ...
    def eval(self, df_tr: pd.DataFrame, scale=1e5):  # use a higher scale for ACD
        try:
            df_tr = df_tr.dropna(subset=['time', 'price', 'vol']).copy()
            # "time" is in seconds (float); compute duration in seconds
...
if bvc_model == "Hawkes":
    ...
    bvc_metrics = model.eval(trades_df, scale=1e5)  # use higher scale for ACD
elif bvc_model == "ACI":
    try:
        trades_df = fetch_trades_kraken(symbol=symbol_bsi1, lookback_minutes=global_lookback_minutes)
    except Exception as e:
    ...
    bvc_metrics = model.eval(trades_df, scale=1e5)  # use higher scale for ACI
else:
